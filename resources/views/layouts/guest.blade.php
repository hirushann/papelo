...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ $title ?? config('app.name', 'Papelooo') }}</title>
        <meta name="description" content="{{ $description ?? 'Papelooo is the ultimate online platform for Sri Lankan students to practice past exam papers as interactive MCQs with instant scoring and topic analytics.' }}">
        ...
        <meta property="og:title" content="{{ $title ?? config('app.name', 'Papelooo') }}">
        <meta property="og:description" content="{{ $description ?? 'Papelooo is the ultimate online platform for Sri Lankan students to practice past exam papers as interactive MCQs with instant scoring and topic analytics.' }}">
        
        <!-- Twitter -->
        <meta property="twitter:card" content="summary_large_image">
        <meta property="twitter:url" content="{{ url()->current() }}">
        <meta property="twitter:title" content="{{ $title ?? config('app.name', 'Papelooo') }}">
        <meta property="twitter:description" content="{{ $description ?? 'Papelooo is the ultimate online platform for Sri Lankan students to practice past exam papers as interactive MCQs with instant scoring and topic analytics.' }}">
    ...
    <body class="bg-examsheet-quiet text-ink antialiased min-h-screen flex flex-col" style="font-family: 'Inter', sans-serif;">
        <!-- SIMPLE HEADER -->
        <header class="px-6 h-16 flex items-center justify-between max-w-6xl mx-auto w-full">
        ...
        <footer class="text-center text-xs text-ink/40 pb-8">&copy; {{ date('Y') }} Papelooo. Made for Sri Lankan students.</footer>
    </body>
</html>
