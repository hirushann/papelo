<div>
  <style>
    ...
    .tab-btn { font-size:0.8rem; font-weight:600; padding:0.4rem 0.9rem; border-radius:9999px; color:rgba(34,49,74,0.5); border:1px solid rgba(34,49,74,0.15); transition: all 0.2s; }
    .tab-btn:hover { border-color:rgba(34,49,74,0.4); }
    .tab-btn.active { background:#22314A; border-color:#22314A; color:#F5F1E6; }
  </style>
  ...
  <main class="max-w-6xl mx-auto px-6 py-10">
    ...
    <h1 class="font-display text-3xl text-ink mb-1">Your Progress</h1>
    <p class="text-ink/60 mb-8">Every attempt, and where to focus next.</p>

    <!-- STATS -->
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
      <div class="bg-white rounded-xl border border-ink/10 p-4">
        <p class="text-xs text-ink/50 mb-1.5">Total attempts</p>
        <p class="font-display text-2xl text-ink">{{ $totalAttempts }}</p>
      </div>
      <div class="bg-white rounded-xl border border-ink/10 p-4">
        <p class="text-xs text-ink/50 mb-1.5">Average score</p>
        <p class="font-display text-2xl text-teal">{{ $averageScore }}%</p>
      </div>
      <div class="bg-white rounded-xl border border-ink/10 p-4">
        <p class="text-xs text-ink/50 mb-1.5">Strongest topic</p>
        <p class="font-display text-lg text-ink truncate" title="{{ $strongestTopic }}">{{ $strongestTopic }}</p>
      </div>
      <div class="bg-white rounded-xl border border-ink/10 p-4">
        <p class="text-xs text-ink/50 mb-1.5">Needs work</p>
        <p class="font-display text-lg text-margin truncate" title="{{ $weakestTopic }}">{{ $weakestTopic }}</p>
      </div>
    </div>

    <!-- SCORE TREND -->
    @if(count($scoreTrend) > 0)
    <div class="bg-white rounded-2xl border border-ink/10 p-6 mb-6">
      <h2 class="font-display text-lg text-ink mb-6">Score trend</h2>
      <div class="flex items-end justify-between gap-3 h-40 mb-1">
        @foreach($scoreTrend as $attempt)
        <div class="flex-1 flex flex-col items-center gap-2">
          <div class="w-full {{ $attempt->score >= 40 ? 'bg-teal' : 'bg-margin' }} rounded-t-md" style="height:{{ max($attempt->score, 5) }}%"></div>
          <span class="text-[10px] text-ink/40 hidden sm:block truncate w-full text-center">{{ substr($attempt->paper->subject->name, 0, 3) }} '{{ substr($attempt->paper->year, -2) }}</span>
        </div>
        @endforeach
      </div>
      <div class="flex items-center gap-4 text-[11px] text-ink/50 pt-3 border-t border-ink/10">
        <span class="flex items-center gap-1.5"><span class="w-2 h-2 rounded-sm bg-teal"></span> Above pass mark (40%)</span>
        <span class="flex items-center gap-1.5"><span class="w-2 h-2 rounded-sm bg-margin"></span> Below pass mark</span>
      </div>
    </div>
    @endif

    <!-- WEAK-AREA BREAKDOWN -->
    @if(count($topicBreakdown) > 0)
    <div class="bg-white rounded-2xl border border-ink/10 p-6 mb-8">
      <h2 class="font-display text-lg text-ink mb-1">Topic breakdown, across all attempts</h2>
      <p class="text-xs text-ink/50 mb-6">Average score per topic — the lowest ones are worth revisiting first.</p>
      <div class="grid sm:grid-cols-2 gap-x-10 gap-y-4">
        @foreach($topicBreakdown as $topicData)
        <div>
          <div class="flex justify-between text-sm mb-1">
            <span class="text-ink/70">{{ $topicData['topic'] }} <span class="text-ink/30">&middot; {{ $topicData['subject'] }}</span></span>
            <span class="font-semibold {{ $topicData['score'] < 60 ? 'text-margin' : 'text-ink' }}">{{ $topicData['score'] }}%</span>
          </div>
          <div class="h-2 rounded-full bg-ink/10">
            <div class="h-2 rounded-full {{ $topicData['score'] < 60 ? 'bg-margin' : 'bg-teal' }}" style="width:{{ $topicData['score'] }}%"></div>
          </div>
        </div>
        @endforeach
      </div>
    ...
    <div class="bg-white rounded-2xl border border-ink/10 overflow-hidden">
      <div class="flex flex-wrap items-center justify-between gap-3 px-6 py-4 border-b border-ink/10">
        <h2 class="font-display text-lg text-ink">Attempt history</h2>
        <div class="flex items-center gap-2">
          <button wire:click="setSubjectFilter('all')" class="tab-btn {{ $subjectFilter === 'all' ? 'active' : '' }}">All</button>
          @foreach($availableSubjects as $subject)
            <button wire:click="setSubjectFilter('{{ $subject }}')" class="tab-btn {{ $subjectFilter === $subject ? 'active' : '' }}">{{ $subject }}</button>
          @endforeach
        </div>
      </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-ink/5">
            ...
            <tr>
              <td class="px-6 py-3.5 text-ink/80">{{ $attempt->paper->title }}</td>
              <td class="px-6 py-3.5"><span class="text-[11px] font-semibold uppercase text-teal bg-teal/10 rounded-full px-2.5 py-0.5">{{ $attempt->paper->subject->level ?? 'N/A' }}</span></td>
              <td class="px-6 py-3.5"><span class="{{ $attempt->score >= 40 ? 'text-teal' : 'text-margin' }} font-semibold">{{ $attempt->score }}%</span></td>
              <td class="px-6 py-3.5 text-ink/40">{{ $attempt->completed_at->isToday() ? 'Today' : $attempt->completed_at->format('M j, Y') }}</td>
              <td class="px-6 py-3.5 text-right"><a href="{{ route('result.summary', $attempt->id) }}" wire:navigate class="text-xs font-semibold text-teal hover:underline">View &rarr;</a></td>
            </tr>
            @endforeach
          </tbody>
        </table>
