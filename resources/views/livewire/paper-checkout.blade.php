<div x-data="payhereCheckout()">
  <style>
    ...
    .field input { width:100%; border:1px solid rgba(34,49,74,0.15); border-radius:0.5rem; padding:0.6rem 0.85rem; font-size:0.875rem; color:#22314A; background:#fff; }
    .field input:focus { outline:none; border-color:#3F7D6B; box-shadow:0 0 0 3px rgba(63,125,107,0.15); }
    .field input[readonly] { background:rgba(34,49,74,0.02); color:rgba(34,49,74,0.5); cursor:not-allowed; }
    .method-badge { font-size:0.7rem; font-weight:600; color:rgba(34,49,74,0.5); border:1px solid rgba(34,49,74,0.15); border-radius:0.375rem; padding:0.3rem 0.6rem; }
  </style>

  <div class="bg-examsheet-quiet text-ink antialiased min-h-screen">
    <!-- MINIMAL HEADER -->
    <header class="px-6 h-16 flex items-center justify-between max-w-xl mx-auto w-full">
    ...
    <!-- MAIN -->
    <main class="max-w-xl mx-auto px-6 py-6">

      <div class="bg-white rounded-2xl border border-ink/10 p-7 mb-5">
        <h1 class="font-display text-xl text-ink mb-6">Checkout</h1>

        <!-- ORDER SUMMARY -->
        <div class="flex items-center justify-between pb-5 mb-5 border-b border-ink/10">
          <div>
            @if($paper->subject)
              <span class="inline-block text-[11px] font-semibold uppercase text-teal bg-teal/10 rounded-full px-2.5 py-0.5 mb-2">{{ strtoupper($paper->subject->level) }}</span>
            @endif
            <p class="font-medium text-ink">{{ $paper->title }}</p>
            <p class="text-xs text-ink/50">{{ $paper->questions()->count() }} questions &middot; Instant marking &middot; Full review</p>
          </div>
          <p class="font-display text-xl text-ink flex-shrink-0">Rs. {{ number_format($paper->price, 0) }}</p>
        </div>

        <div class="flex justify-between items-center pb-6 mb-6 border-b border-ink/10">
          <span class="text-sm text-ink/60">Total due today</span>
          <span class="font-display text-2xl text-ink">Rs. {{ number_format($paper->price, 2) }}</span>
        </div>

        <!-- BILLING INFO (Readonly from auth) -->
        ...
        <!-- PAYMENT -->
        <div class="rounded-xl border border-ink/10 bg-paper/50 p-5 mb-6">
          <div class="flex items-center gap-2 mb-3">
            ...
            <p class="text-sm font-medium text-ink">Payment processed securely by PayHere</p>
          </div>
          <p class="text-xs text-ink/50 mb-3">You'll be taken to PayHere's secure payment page to complete your purchase. Papelo never sees or stores your card details.</p>
          <div class="flex flex-wrap gap-2">
            <span class="method-badge">Visa</span>
            <span class="method-badge">Mastercard</span>
            <span class="method-badge">eZ Cash</span>
            <span class="method-badge">mCash</span>
            <span class="method-badge">Online Banking</span>
          </div>
        </div>

        <button wire:click="processPayment" wire:loading.attr="disabled" class="w-full flex justify-center items-center gap-2 rounded-lg bg-teal text-paper font-semibold py-3.5 hover:bg-teal/90 transition disabled:opacity-50 disabled:cursor-not-allowed">
          <span wire:loading.remove>Pay Rs. {{ number_format($paper->price, 0) }} securely</span>
          <span wire:loading>Redirecting securely...</span>
        </button>
        <p class="text-center text-xs text-ink/40 mt-3">By paying, you agree to Papelo's <a href="#" class="underline">Terms</a> and <a href="#" class="underline">Refund Policy</a>.</p>
      </div>
  ...
  <script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('payhereCheckout', () => ({
            init() {
                this.$wire.on('initiate-payhere', (data) => {
                    const params = data[0];
                    ...
                    // Create hidden inputs for all params except url
                    for (const key in params) {
                        if (key !== 'url' && params.hasOwnProperty(key)) {
