<div>
  ...
  <header class="px-6 h-16 flex items-center justify-between max-w-3xl mx-auto w-full">
    ...
    <a href="{{ route('dashboard') }}" wire:navigate class="text-sm font-medium text-ink/60 hover:text-ink">Dashboard &rarr;</a>
  </header>
  ...
  <main class="max-w-3xl mx-auto px-6 py-10">
    ...
    <div class="text-center mb-10">
      <p class="text-xs font-semibold uppercase tracking-wide text-ink/40 mb-2">{{ $attempt->paper->title ?? 'Exam' }} — Results</p>
      ...
      <div class="relative w-52 h-52 mx-auto mb-6">
        ...
        <div class="absolute inset-0 flex flex-col items-center justify-center">
          <span class="font-display text-5xl text-ink">{{ $percentage }}%</span>
          <span class="text-xs text-ink/50 mt-1">{{ $correctCount }} / {{ $totalQuestions }} correct</span>
        </div>
      </div>

      @if($percentage >= 40)
      <span class="inline-flex items-center gap-1.5 text-sm font-semibold text-teal bg-teal/10 rounded-full px-4 py-1.5 mb-2">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M20 6L9 17l-5-5"/></svg>
        Pass — well above the 40% pass mark
      </span>
      @else
      <span class="inline-flex items-center gap-1.5 text-sm font-semibold text-margin bg-margin/10 rounded-full px-4 py-1.5 mb-2">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M18 6L6 18M6 6l12 12"/></svg>
        Needs improvement — below the 40% pass mark
      </span>
      @endif
      <p class="text-sm text-ink/50">Completed in {{ $timeTakenMinutes }}m {{ $timeTakenSeconds }}s, out of {{ $attempt->paper->duration_minutes ?? 60 }} minutes</p>
    </div>

    <!-- STAT CHIPS -->
    <div class="grid grid-cols-3 gap-4 mb-8">
      <div class="bg-white rounded-xl border border-ink/10 p-4 text-center">
        <p class="font-display text-2xl text-teal">{{ $correctCount }}</p>
        <p class="text-xs text-ink/50 mt-1">Correct</p>
      </div>
      <div class="bg-white rounded-xl border border-ink/10 p-4 text-center">
        <p class="font-display text-2xl text-margin">{{ $incorrectCount }}</p>
        <p class="text-xs text-ink/50 mt-1">Incorrect</p>
      </div>
      <div class="bg-white rounded-xl border border-ink/10 p-4 text-center">
        <p class="font-display text-2xl text-ink/40">{{ $unansweredCount }}</p>
        <p class="text-xs text-ink/50 mt-1">Unanswered</p>
      </div>
    </div>

    <!-- TOPICS TO REVISIT -->
    @if(count($topicAnalytics) > 0)
    <div class="bg-white rounded-2xl border border-ink/10 p-6 mb-8">
      ...
      <div class="space-y-4">
        ...
        @php
            $topicColor = $data['percentage'] >= 75 ? 'teal' : ($data['percentage'] <= 40 ? 'margin' : 'gold');
        @endphp
        <div>
          <div class="flex justify-between text-sm mb-1"><span class="text-ink/70">{{ $topic }}</span><span class="font-semibold text-{{ $topicColor }}">{{ $data['percentage'] }}%</span></div>
          <div class="h-2 rounded-full bg-ink/10"><div class="h-2 rounded-full bg-{{ $topicColor }}" style="width:{{ $data['percentage'] }}%"></div></div>
        </div>
        @endforeach
      </div>
    </div>
