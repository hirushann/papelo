...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-paper text-ink antialiased font-sans">
        ...
        <header class="sticky top-0 z-50 bg-paper/95 backdrop-blur-sm border-b border-ink/10">
            <div class="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
                <a href="/" class="flex items-center gap-2">
                    ...
                    <span class="font-display text-2xl text-ink">Papelo</span>
                </a>
        ...
        {{-- ── Pricing ────────────────────────────────────────── --}}
        <section id="pricing" class="bg-examsheet-quiet">
            <div class="max-w-6xl mx-auto px-6 py-24">
                <h2 class="font-display text-3xl text-ink text-center mb-4">Simple, exam-friendly pricing</h2>
                <p class="text-ink/60 text-center mb-14">Placeholder amounts below — swap in your real pricing.</p>
                <div class="grid md:grid-cols-2 gap-6 max-w-2xl mx-auto">
                    <div class="rounded-2xl border border-ink/10 bg-white p-8">
                        <h3 class="font-display text-xl text-ink mb-1">Pay per paper</h3>
                        <p class="text-sm text-ink/60 mb-6">Good for trying a subject before committing.</p>
                        <p class="mb-6"><span class="font-display text-4xl text-ink">Rs. 100</span><span class="text-ink/50 text-sm"> / paper</span></p>
                        <ul class="space-y-2.5 text-sm text-ink/70 mb-8">
                            <li class="flex items-center gap-2"><span class="w-1.5 h-1.5 rounded-full bg-teal"></span> One paper, one attempt</li>
                            <li class="flex items-center gap-2"><span class="w-1.5 h-1.5 rounded-full bg-teal"></span> Instant marking</li>
                            <li class="flex items-center gap-2"><span class="w-1.5 h-1.5 rounded-full bg-teal"></span> Full topic breakdown</li>
                        </ul>
                        <a href="{{ auth()->check() ? route('papers') : route('register') }}" class="block text-center rounded-lg border border-ink/20 text-ink font-semibold py-3 hover:bg-ink hover:text-paper transition">Get started</a>
                    </div>
                    <div class="rounded-2xl border-2 border-teal bg-white p-8 relative">
                        <span class="absolute -top-3 right-8 bg-teal text-paper text-xs font-semibold rounded-full px-3 py-1">Best value</span>
                        <h3 class="font-display text-xl text-ink mb-1">Unlimited monthly</h3>
                        <p class="text-sm text-ink/60 mb-6">Best during exam season.</p>
                        <p class="mb-6"><span class="font-display text-4xl text-ink">Rs. 990</span><span class="text-ink/50 text-sm"> / month</span></p>
                        <ul class="space-y-2.5 text-sm text-ink/70 mb-8">
                            <li class="flex items-center gap-2"><span class="w-1.5 h-1.5 rounded-full bg-teal"></span> Unlimited papers, unlimited attempts</li>
                            <li class="flex items-center gap-2"><span class="w-1.5 h-1.5 rounded-full bg-teal"></span> Full progress history</li>
                            <li class="flex items-center gap-2"><span class="w-1.5 h-1.5 rounded-full bg-teal"></span> Priority new-paper access</li>
                        </ul>
                        <a href="{{ auth()->check() ? route('papers') : route('register') }}" class="block text-center rounded-lg bg-teal text-paper font-semibold py-3 hover:bg-teal/90 transition">Get started</a>
                    </div>
                </div>
            </div>
        </section>
