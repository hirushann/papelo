...
results with a topic-by-topic weak-area breakdown. Monetized via monthly
subscription plans (Practice, Progress, Pass) through Lemon Squeezy.
Differentiator vs. free PDF competitors: instant scoring + topic analytics,
not just access to the papers.
...
- **Client-side interactivity:** Alpine.js (ships with Flux/Livewire) — used
  ...
  countdown timer
- **Database:** MySQL
- **Payments:** Lemon Squeezy (monthly subscriptions) — webhook signature
  verification uses HMAC-SHA256 via X-Signature header
...
plans           (id, name, slug, ls_variant_id, price, paper_limit, features, sort_order, is_active)
subscriptions   (id, user_id FK, plan_id FK, ls_subscription_id, ls_customer_id, status[active|cancelled|past_due|expired|paused], current_period_start, current_period_end, attempts_used, cancelled_at)
purchases       (id, user_id FK, paper_id FK, amount_paid, payhere_order_id, status — LEGACY, kept for historical data)
...
component (QuizTaker, ResultSummary, PaperCatalog, SubscriptionCheckout,
Admin/QuestionManager) depends on these exact names.
...
- Never commit `.env`
- Payment webhook handling must verify Lemon Squeezy's HMAC-SHA256 signature
  (X-Signature header) before trusting any status update — do not create or
  update a Subscription record without signature verification
...
- [x] Prompt 6 (PayHere integration) — replaced with Lemon Squeezy subscriptions
- [x] Prompt 7 (Landing page)
- [x] Prompt 8 (Lemon Squeezy subscription migration)
